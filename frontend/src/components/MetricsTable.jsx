...
function toDetailLines(weekly_detail) {
  if (Array.isArray(weekly_detail)) return weekly_detail.filter(Boolean);
  return weekly_detail ? weekly_detail.split("\n").map(l => l.trim()).filter(Boolean) : [];
}
      ...
      {indicators.map((ind, i) => {
        ...
        return (
          <tr key={i} className="metrics-table__indicator-row">
            ...
            <td><span className="ind-pct-badge" style={pctStyle(pct)}>{pct}%</span></td>
            <td>
...
export function ProjectMetricsTable({ project }) {
  ...
  const nonConf    = (project.impediments || []).filter(im => im.category === "non_conformity");
              ...
              <td colSpan={3}>
                ...
                {shared > 0 && (
                  <span style={{ fontWeight: 400, fontSize: "11px", marginLeft: 10, opacity: 0.85 }}>
                    ({engineers.reduce((s, e) => s + Number(e.assigned || 0), 0)} asignadas − {shared} compartidas = {engineers.reduce((s, e) => s + Number(e.assigned || 0), 0) - shared} reales)
                  </span>
                )}
              </td>
            </tr>
